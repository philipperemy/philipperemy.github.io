...
We consider a stateless LSTM in our first experiment and the tuple $$(X, Y)$$. Our objective is to prove that if we split the big sequences contained in $$X$$ into smaller sequence, the model cannot converge. The function that we use to split a big sequence in a sequence of sequence is:
...
For `window_length=10`, the shapes become:
The shape of the matrices are:
